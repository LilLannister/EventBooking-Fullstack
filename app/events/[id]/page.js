...
export default async function EventDetailPage({ params }) {
    ...
    const soldTickets = event._count?.bookings || 0;
    const soldOut = soldTickets >= event.capacity;
            ...
            <section className="mx-auto max-w-4xl">
                <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
                    ...
                    <div className="mt-8 flex flex-wrap items-start gap-4">
                        <BookTicketButton eventId={event.id} soldOut={soldOut} />

                        <Link
                            href="/events"
                            className="inline-flex h-fit rounded-xl border border-slate-300 bg-white px-6 py-3 font-semibold text-slate-700 transition hover:bg-slate-100"
                        >
                            Back to Events
                        </Link>
                    </div>
